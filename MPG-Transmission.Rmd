---
title: "Fuel vs. transmission: weight matters!"
output: pdf_document
---
...
```{r, results='hide'}
mymtcars <- mtcars
mymtcars$am <- gsub(0, 'Automatic', mymtcars$am)
mymtcars$am <- gsub(1, 'Manual',    mymtcars$am)
...
Note that both including weight (in fit2) and horsepower (in fit3) provides a significant improvement, with p-values of $5 \times 10^{-9}$ and $5 \times 10^{-4}$ respectively in their likelihood ratio tests compared to models not including them. Including more variables, however, does not significantly improve our model any further, since all associated p-values are larger than 5% (other factors have been considered as well, though they are not shown here for brevity):

```{r}
fit4 <- lm(mpg ~ am + wt + hp + factor(cyl), mymtcars)
fit5 <- lm(mpg ~ am + wt + hp + disp,        mymtcars)

cbind(anova(fit3, fit4)$'Pr(>F)'[-1],
      anova(fit3, fit5)$'Pr(>F)'[-1])
```
...
There are no drastic outliers to the regression curve, and the minor outliers are slightly, but not excessively more influential than the typical data point, as one can see by comparing their hat values --- `r hatvalues(fit3)[c('Toyota Corolla', 'Chrysler Imperial', 'Fiat 128')]`, respectively --- to the mean hat value of `r mean(hatvalues(fit3))` of the fit.